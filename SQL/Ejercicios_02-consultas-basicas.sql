...
select "Name" as "Track_Title" from "Track" where "Milliseconds" >= 300.000;
...
select "FirstName" as "Name", "Email" as "Email_Address" from "Customer" where "Country" = 'Brazil';
...
select "Name" as "Playlist_Name" from "Track" where "Name" like '%Music%';
...
select "FirstName" as "First_Name", "LastName" as "Last_Name" from "Employee" where "Title" = 'Sales Support Agent';
...
select "FirstName" as "First_Name", "LastName" as "Last_Name" from "Employee" where "ReportsTo" = 1;
...
select "Name" as "Track_Name", "Milliseconds" as "Duration_Minutes" from "Track" where "Milliseconds" > 300.000;
...
select "Name", "Milliseconds" from "Track" where "UnitPrice" > 1 and "Milliseconds" < 300.000;
...
select "FirstName", "LastName" from "Employee" where "City" = 'Seattle' and "State" = 'WA';
...
select "FirstName", "CustomerId" from "Customer" where "Country" = 'USA' and "SupportRepId" >= 5;
...
select "Name", "MediaTypeId" from "Track" where "Milliseconds" > 250.000 and "MediaTypeId" = 1 or "MediaTypeId" = 2;
